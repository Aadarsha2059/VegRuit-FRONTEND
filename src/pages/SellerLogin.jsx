...
const SellerLogin = ({ onAuthSuccess }) => {
  ...
  React.useEffect(() => {
    const storedLockout = localStorage.getItem('sellerLoginLockout');
    if (storedLockout) {
      ...
      if (now < lockoutData.until) {
        ...
        localStorage.removeItem('sellerLoginLockout');
      }
    }
    
    const storedAttempts = localStorage.getItem('sellerLoginAttempts');
    if (storedAttempts) {
    ...
    if (lockoutTime) {
      const interval = setInterval(() => {
        ...
        if (remaining === 0) {
          ...
          setLoginAttempts(0);
          localStorage.removeItem('sellerLoginLockout');
          localStorage.removeItem('sellerLoginAttempts');
        }
      }, 1000);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        localStorage.setItem('sellerLoginAttempts', newAttempts.toString());
        
        // Lock account after 15 failed attempts
        if (newAttempts >= 15) {
          const lockUntil = Date.now() + (10 * 60 * 1000); // 10 minutes
          setLockoutTime(lockUntil);
          localStorage.setItem('sellerLoginLockout', JSON.stringify({
            until: lockUntil,
  ...
  return (
    <>
      ...
      <form onSubmit={handleLogin} className="auth-form">
        ...
        <button type="submit" className="submit-btn seller-btn" disabled={loading || (lockoutTime && remainingTime > 0)}>
